...
--
-- Brute force for all possibilities
module Main where
...
main = do
  ...
  let environment = getPositionAndMatrix input
      totalCost = runReader calculateTotalCost environment
      --nextAction = runReader determineNextAction environment
  --putStrLn (show nextAction)
  --print totalCost
  print $ take 20 $ map (\((cost, _), _) -> cost) totalCost

calculateTotalCost :: Reader Env [((Int, Maybe Entry), Matrix)]
calculateTotalCost = do
  Env {envPosition=position, envMatrix=matrix} <- ask
  let dirtyEntries = filter (\(_, v) -> v == dirtyValue) matrix
      allPaths = permutations dirtyEntries
      allPathsWithCost = map (\path -> mapAccumL costWithPath (0, Nothing) path) allPaths
      sortedPaths = sortBy compareCost allPathsWithCost
  return sortedPaths

  where
    costWithPath (cost, Nothing) entry = ((cost, Just entry), entry)
    costWithPath (cost, Just lastEntry) entry = ((cost + (calculateCost lastEntry entry), Just entry), entry)

    calculateCost ((x, y), _) ((x', y'), _) = (abs $ x - x') + (abs $ y - y')

    compareCost ((cost, _), _) ((cost', _), _) = cost `compare` cost'
